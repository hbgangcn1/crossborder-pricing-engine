...
def main():
    ...
    if not any([args.unit, args.integration, args.security,
                args.performance, args.comprehensive]):
        args.unit = args.integration = args.security = \
            args.performance = args.comprehensive = True
    ...
    if args.coverage:
        total_count += 1
        print("\n📊 生成覆盖率报告...")
        cmd = (f"python -m pytest {' '.join(test_files)} "
               f"--cov=. --cov-report=html --cov-report=term")
